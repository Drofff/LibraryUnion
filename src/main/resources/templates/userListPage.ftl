...
<nav class="navbar navbar-expand-lg navbar-primary bg-primary">
    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
        ...
        <div class="form-inline ml-4" style="width: 10%;">
            <form action="/admin/usrList" method="get">
            <input id="search_field" name="search" autocomplete="off" class="form-control form-control-dark" type="text" placeholder="Search by username" <#if query??>value = "${query}"</#if> aria-label="Search">
            </form>
        </div>
    </div>
...
<div>
    <table class="table table-bordered">
        ...
        <tbody>
        <#list users as user, hasLibraryCard>
        <tr>
            <td>${user.username}</td>
            <#if hasLibraryCard>
            <td><a href="/admin/statistic?user=${user.id}">Review</a></td>
            <#else>
            <td><a href="/admin/block/user/${user.id}">Delete</a></td>
            </#if>
        </tr>
        </#list>
        </tbody>
